                 ...
                 const uint64_t lenght)
{
    ...
    for (i = 0; i < size - 1; i = i + 2) {
        ...
        */
         ...
         */
        uint32x4_t X =
            vreinterpretq_u32_u8(veorq_u8(block_x,   generate_key_x));
        uint32x4_t Y =
            vreinterpretq_u32_u8(veorq_u8(block_y, generate_key_y));

        /*
         * Update the internal hash state using polynomial
         * multiplications and integer accumulation.
         */
        update_function(X, Y, roundKeys_zero, roundKeys_ones, output);
